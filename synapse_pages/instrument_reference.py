"""Instrument Reference — Analytical Data Bank browser (wraps analytical_databank/app.py)."""
...
def render():
    # ── Header ──
    st.title("Instrument Reference")
    st.caption(
        "Comprehensive reference for analytical instrumentation in Biotech & Biopharma "
        "— Principles, Industry Models, Methods by Product, and Data Reporting Standards"
    )

    # ── Sidebar Navigation ──
    st.sidebar.markdown("---")
    st.sidebar.subheader("Instrument Browser")
    ...
    selected_category = st.sidebar.selectbox(
        "Select Category", categories, key="adb_category"
    )
    ...
    selected_instrument = st.sidebar.selectbox(
        "Select Instrument", instrument_names, key="adb_instrument"
    )

    # Quick search
    st.sidebar.markdown("---")
    search_term = st.sidebar.text_input("Search across all instruments", "", key="adb_search")
    ...
    if search_term.strip():
        ...
        st.header(data.get("full_name", selected_instrument))
        ...
        with tab3:
            ...
            if methods_by_product:
                ...
                selected_product = st.selectbox(
                    "Select Product Type", product_types, key="adb_product"
                )
                methods = methods_by_product[selected_product]
                for i, method in enumerate(methods):
                    with st.expander(
                        f"{method['method']} — {method['purpose']}", expanded=(i == 0)
                    ):
